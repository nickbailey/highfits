...
### estimating B
def estimate_B(xs, ys, initial_bin_f0, initial_B):
    xs = numpy.array(xs)
    ys = numpy.array(ys)
    if len(xs) < 2:
        return initial_bin_f0, initial_B, 1.0
...
def estimate_f0_B(filenames):
    ...
    bin_spread_below = int(
        stft.hertz2bin(defs.B_PEAK_SPREAD_BELOW_HERTZ*base_frequency_estimate,
            sample_rate))
    bin_spread_above = int(
        stft.hertz2bin(defs.B_PEAK_SPREAD_ABOVE_HERTZ*base_frequency_estimate,
            sample_rate))
...
def get_header():
    return "name\tnum files\t$f_0$\t$B$\t$R^2$\thighest detected\tstiff-ideal limit\tnotes"
