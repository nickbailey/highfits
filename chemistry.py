...
def fit_lorentzian(ffta, bin_low, bin_high, bin_f0, plot=False,
        add_plot=False):
    show = True
    ...
    w = numpy.array( [
        (bin_low+i)*0.5*defs_measure.INPUT_RATE / float(len(ffta)) for i, a in enumerate(peak_area)
        ] )
    ...
    try:
        ...
        return None, 0, 0, 0, 0
    #print "Rsquared, var", rsquared, variance
    freq = p[1]

    decay = 2*numpy.pi / abs(p[0])

    mag = p[2]
    #print "Detected: %.1f   %.2f   %.2g" % (freq, decay, mag)
    ...
    if plot:
        ...
        pylab.title("Searching for mode around %.5g" % (bin_f0))
    ...
    if add_plot:
        ...
        #pylab.plot(w_eval, (predicted),
            ...
            color="red")
    return freq, decay, mag, rsquared, variance
...
def calc_harmonics(wav_filename, f_0=None, B=None, plot=False):
    ...
    search_radius = LONG_PEAK_AREA_HERTZ
    ...
    for n in range(1,NODES_MAXIMUM+1):
        ...
        w = 2*numpy.pi*freq
        Q = w / (2*alpha)
        decay = classes.Decay(freq, w, n, alpha, Q, rsquared, variance)
        decays.append(decay)
    if add_plot:
        pylab.show()
    return decays
